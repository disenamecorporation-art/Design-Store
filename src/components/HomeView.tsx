...
import { TabType, ServiceItem, PortfolioProject } from '../types';
import { SERVICES_DATA } from '../data';
import { getPortfolioProjects } from '../data/portfolioStore';
import { ArrowRight, Sparkles, CheckCircle2, Send, PenTool, Printer, Layers, Zap, MapPin, Mail, Phone, Briefcase, Calendar, Users, Award, TrendingUp, Calculator, Search, Star, ExternalLink } from 'lucide-react';
...
export const HomeView: React.FC<HomeViewProps> = ({ setActiveTab, onSearchTracking }) => {
  ...
  return (
    <div className="min-h-screen bg-[#fbfbfd] text-zinc-900 font-sans selection:bg-black selection:text-white overflow-x-hidden">
      ...
      {/* Slider de Portafolio en el Home */}
      <section className="py-24 bg-[#fbfbfd] overflow-hidden border-t border-zinc-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 mb-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4">
            <div>
              <span className="text-xs font-bold text-amber-500 uppercase tracking-widest">Nuestra Experiencia</span>
              <h3 className="text-3xl sm:text-4xl font-extrabold text-zinc-900 mt-1">Proyectos que inspiran.</h3>
            </div>
            <button
              onClick={() => {
              ...
              className="text-sm font-bold text-zinc-950 hover:text-amber-500 flex items-center gap-1.5 transition-colors group/btn"
            >
        ...
        {portfolioProjects.length > 0 ? (
          <div className="relative overflow-hidden w-full py-4 bg-gradient-to-r from-[#fbfbfd] via-transparent to-[#fbfbfd] group">
            ...
            <div className="animate-marquee gap-6 flex items-center">
              {[...portfolioProjects, ...portfolioProjects, ...portfolioProjects].map((project, idx) => (
                <div
                  key={`${project.id}-h1-${idx}`}
                  onClick={() => {
                    setActiveTab('portafolio');
                    window.scrollTo({ top: 0, behavior: 'smooth' });
                  }}
                ...
                >
                  ...
                  {/* ONLY Title and Client Name Header */}
                  <div className="p-5 bg-white space-y-1">
                    <h3 className="text-base font-extrabold text-zinc-900 line-clamp-1 group-hover/card:text-amber-500 transition-colors">
                      {project.title}
                    </h3>
                    {project.clientName && (
                  ...
                  <div className="space-y-6 pt-4 px-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-zinc-500 font-medium text-sm mt-0.5">Zona Industrial Carabobo 2 Nort Suiza, Bello monte 2 Av. Pinto salinas</p>
                      </div>
                    </div>
